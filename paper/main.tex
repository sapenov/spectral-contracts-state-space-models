...
Our evaluation uses controlled synthetic matrices rather than parameters from actual trained models, which limits direct external validity. As a preliminary bridge, we evaluated our diagnostics on $n=30$ matrices constructed using DSS-style initialization: diagonal eigenvalues in $[0.5, 0.99]$ with random orthogonal rotations introducing moderate non-normality ($\kappa(V) \in \{1, 2, 5, 10, 20\}$). C3 values on these initialized matrices fell in the range $[0.091, 0.511]$ (mean: $0.312$), consistent with the recurrence-inspired regime where our benchmark finds pseudospectral analysis informative. The rank correlation between C3 and linear dynamical growth ratio on these initialized matrices was $\rho = 0.890$, qualitatively consistent with the recurrence-inspired benchmark findings.

As a further bridge to real architectures, we generated operators using the S4D-Real initialization scheme \citep{gu2022efficiently} with learned rotations simulating post-initialization training updates. On $n = 30$ operators (hidden size $= 64$, 30 random seeds with varying rotation strengths), C3 achieved Spearman $\rho = 0.269$ (95\% bootstrap CI: [-0.151, 0.589]), weaker than the synthetic benchmark but in the predicted direction. C3 values ranged [0.031, 0.195] (mean: 0.086), confirming that realistic S4/DSS operators at initialization occupy a regime where pseudospectral analysis provides weak but directionally consistent signal, though the correlation is attenuated compared to our controlled non-normal benchmark.

Full validation on actual SSM training trajectories — including post-training checkpoints and training-time instability events — remains important future work.
...
\textbf{Reproducibility Statement}: All code, data, and experimental configurations are available at \url{https://github.com/sapenov/spectral-contracts-state-space-models}. The synthetic matrix generation, contract computations, and statistical analyses can be reproduced using the provided scripts.
